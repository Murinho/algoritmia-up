...
import { updateContest, deleteContest } from '@/lib/contests';
...
export default function ContestUpdateDialog({
  open,
  contest,
  onClose,
  onUpdated,
  ...
  // Prefill when opening / contest changes
  useEffect(() => {
    if (open && contest) {
      setTitle(contest.title ?? '');
      setPlatform(contest.platform ?? 'Codeforces');
      setUrl(contest.url ?? '');
      setTags(contest.tags ?? []);
      setTagInput('');
      setDifficulty((contest.difficulty as Difficulty) ?? 3);
      setFormat(contest.format ?? 'ICPC');
      setStartsAtLocal(isoToLocalInput(contest.startsAt as unknown as string));
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      const updated = await updateContest(contest.id.toString(), {
        title: title.trim(),
        platform,
        url: url.trim(),
        tags,
      ...
      if (err instanceof HttpError) {
        if (err.status === 401) {
          setError('Debes iniciar sesión para actualizar concursos.');
        } else if (err.status === 403) {
          setError('Solo coaches y admins pueden actualizar concursos.');
        } else if (err.message) {
          setError(String(err.message));
        } else {
          setError('Ocurrió un error al actualizar el concurso.');
        }
      } else {
        setError('Ocurrió un error inesperado al actualizar el concurso.');
      }
    } finally {
      setSubmitting(false);
    }
  ...
  async function handleDelete() {
    if (!contest) return;
    if (!window.confirm('¿Estás seguro de que deseas eliminar este concurso? Esta acción es permanente.')) {
      return;
    }
    ...
    setError(null);
    try {
      await deleteContest(contest.id.toString());
      onDeleted(contest.id.toString());
      onClose();
    } catch (err: unknown) {
      if (err instanceof HttpError) {
        if (err.status === 401) {
          setError('Debes iniciar sesión para eliminar concursos.');
        } else if (err.status === 403) {
          setError('Solo coaches y admins pueden eliminar concursos.');
        } else if (err.message) {
          setError(String(err.message));
        } else {
          setError('Ocurrió un error al eliminar el concurso.');
        }
      } else {
        setError('Ocurrió un error inesperado al eliminar el concurso.');
      }
    } finally {
      setDeleting(false);
    }
        ...
        <div className="relative bg-zinc-900">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-zinc-100 font-semibold">Editar Concurso</h2>
            </div>
            <button
              onClick={onClose}
          ...
          >
            ...
            <div className="pt-3 mt-2 border-t border-white/10 flex items-center justify-between gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-red-500/60 text-red-200 bg-red-500/10 hover:bg-red-500/20 disabled:opacity-60 transition"
              >
                <Trash2 size={16} />
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-zinc-300 border border-white/10 hover:bg-white/5 disabled:opacity-60 transition"
                >
                  Cancelar
                </button>
                <button
                ...
                >
                  {submitting ? 'Actualizando…' : 'Actualizar Concurso'}
                </button>
              </div>
            </div>
          </form>
